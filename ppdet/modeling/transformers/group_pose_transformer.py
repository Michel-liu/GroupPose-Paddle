...
__all__ = ['GroupPoseTransformer']


class GroupPoseTransformerEncoderLayer(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
                 ...
                 weight_attr=None,
                 bias_attr=None):
        super(GroupPoseTransformerEncoderLayer, self).__init__()
        # self attention
...
class GroupPoseTransformerEncoder(nn.Layer):
    def __init__(self, encoder_layer, num_layers):
        super(GroupPoseTransformerEncoder, self).__init__()
...
class GroupPoseTransformerDecoderLayer(nn.Layer):
    def __init__(self,
                 d_model=256,
                 n_head=8,
                 dim_feedforward=1024,
                 ...
                 weight_attr=None,
                 bias_attr=None):
        super(GroupPoseTransformerDecoderLayer, self).__init__()

        # within-instance self-attention
        self.within_attn = MultiHeadAttention(d_model, n_head, dropout=dropout)
        self.within_dropout = nn.Dropout(dropout)
        self.within_norm = nn.LayerNorm(
            d_model,
            weight_attr=ParamAttr(regularizer=L2Decay(0.0)),
            bias_attr=ParamAttr(regularizer=L2Decay(0.0)))
        
        # across-instance self-attention
        self.across_attn = MultiHeadAttention(d_model, n_head, dropout=dropout)
        self.across_dropout = nn.Dropout(dropout)
        self.across_norm = nn.LayerNorm(
            d_model,
            weight_attr=ParamAttr(regularizer=L2Decay(0.0)),
            bias_attr=ParamAttr(regularizer=L2Decay(0.0)))
    ...
    def forward_ffn(self, tgt):
        return self.linear2(self.dropout2(self.activation(self.linear1(tgt))))
    ...
    def forward(self,
        ...
        tgt2 = self.within_attn(q.flatten(0, 1), k.flatten(0, 1), tgt.flatten(0, 1)).reshape(q.shape)
        tgt = tgt + self.within_dropout(tgt2)
        tgt = self.within_norm(tgt)
        
        # across-instance self-attention
        q = k = tgt.transpose([0, 2, 1, 3])
        tgt2 = self.across_attn(q.flatten(0, 1), k.flatten(0, 1), tgt.transpose([0, 2, 1, 3]).flatten(0, 1)).reshape(q.shape).transpose([0, 2, 1, 3])
        tgt = tgt + self.across_dropout(tgt2)
        tgt = self.across_norm(tgt)

        # deformable cross-attention
        bs, nq, _, np = reference_points.shape[:4]
        reference_points = reference_points.transpose([0, 1, 3, 2, 4]).flatten(1, 2)
        tgt2 = self.cross_attn(
            self.with_pos_embed(tgt, query_pos_embed).flatten(1, 2), reference_points, memory,
            memory_spatial_shapes, memory_level_start_index, memory_mask).reshape([bs, nq, np, -1])
...
class GroupPoseTransformerDecoder(nn.Layer):
    def __init__(self,
                 hidden_dim,
                 decoder_layer,
                 num_layers,
                 return_intermediate=True):
        super(GroupPoseTransformerDecoder, self).__init__()
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            reference_points_input = F.sigmoid(ref_points_unact).unsqueeze(
                2) * valid_ratios.unsqueeze(1).unsqueeze(-2)
            bs, nq = ref_points_unact.shape[:2]
            refpoint_pose_reshape = reference_points_input[:, :, 0].flatten(0, 1)
            query_pos_embed = get_sine_pos_embed(
                refpoint_pose_reshape, self.hidden_dim // 2).reshape([bs, nq, -1, self.hidden_dim])
            query_pos_embed = query_pos_head(query_pos_embed)
            output = layer(output, reference_points_input, memory,
                           memory_spatial_shapes, memory_level_start_index,
                           memory_mask, query_pos_embed[:, :, 1:])

            # iteration
            inter_ref_keypoint_unact = pose_head[i](output[:, :, 1:]) + ref_points_unact[:, :, 1:]
            # center of pose
            inter_ref_center_unact = inverse_sigmoid(paddle.mean(F.sigmoid(inter_ref_keypoint_unact), axis=2, keepdim=True))
            inter_ref_keypoint_unact = paddle.concat([inter_ref_center_unact, inter_ref_keypoint_unact], axis=2)

            if self.return_intermediate:
                intermediate.append(output)
                inter_ref_pose.append(inter_ref_keypoint_unact)

            ref_points_unact = inter_ref_keypoint_unact.detach()
...
@register
class GroupPoseTransformer(nn.Layer):
    ...
    def __init__(self,
                 ...
                 eps=1e-2,
                 num_body_points=17):
        super(GroupPoseTransformer, self).__init__()
        ...
        # Transformer module
        encoder_layer = GroupPoseTransformerEncoderLayer(
            hidden_dim, nhead, dim_feedforward, dropout, activation, num_levels,
            num_encoder_points)
        self.encoder = GroupPoseTransformerEncoder(encoder_layer, num_encoder_layers)
        decoder_layer = GroupPoseTransformerDecoderLayer(
            hidden_dim, nhead, dim_feedforward, dropout, activation, num_levels,
            num_decoder_points)
        self.decoder = GroupPoseTransformerDecoder(hidden_dim, decoder_layer,
                                              num_decoder_layers,
        ...
        self.query_pos_head = MLP(hidden_dim,
                                  hidden_dim,
                                  hidden_dim,
                                  num_layers=2)
        ...
        self.enc_keypoint_head = MLP(hidden_dim, 2*hidden_dim, 2*num_body_points, num_layers=4)
        # decoder head
        self.dec_score_head = nn.LayerList([
            nn.Linear(hidden_dim, num_classes)
            for _ in range(num_decoder_layers)
        ])
        self.dec_keypoint_head = nn.LayerList([
    ...
    def _reset_parameters(self):
        # class and keypoint head init
        bias_cls = bias_init_with_prob(0.01)
        linear_init_(self.enc_score_head)
        constant_(self.enc_score_head.bias, bias_cls)
        constant_(self.enc_keypoint_head.layers[-1].weight)
        constant_(self.enc_keypoint_head.layers[-1].bias)
        for cls_, reg_ in zip(self.dec_score_head, self.dec_keypoint_head):
            linear_init_(cls_)
        ...
        for l in self.input_proj:
            ...
            constant_(l[0].bias)
        # embedding
        xavier_uniform_(self.keypoint_embedding.weight)
        xavier_uniform_(self.instance_embedding.weight)
    ...
    def forward(self, feats, pad_mask=None):
        ...
        target, init_ref_points_unact, enc_topk_keypoints, enc_topk_logits = \
            self._get_decoder_input(memory, spatial_shapes, mask_flatten)

        # decoder
        inter_feats, inter_ref_keypoints_unact = self.decoder(
            target, init_ref_points_unact, memory, spatial_shapes,
            level_start_index, self.dec_keypoint_head, self.query_pos_head,
            valid_ratios, mask_flatten)
        
        out_keypoints = []
        out_logits = []
        for i in range(self.num_decoder_layers):
            out_logits.append(self.dec_score_head[i](inter_feats[i][:, :, 0]))
            # print(f"{i} layers: logits_features:{inter_feats[i][:, :, 0]}")
            if i == 0:
                out_keypoints.append(
                    F.sigmoid(self.dec_keypoint_head[i](inter_feats[i][:, :, 1:]) +
                              init_ref_points_unact[:, :, 1:]))
                # print(f"{i} layers: keypoint_features:{inter_feats[i][:, :, 1:]}; base:{init_ref_points_unact[:, :, 1:]}")
            else:
                out_keypoints.append(
                    F.sigmoid(self.dec_keypoint_head[i](inter_feats[i][:, :, 1:]) +
                              inter_ref_keypoints_unact[i - 1][:, :, 1:]))
                # print(f"{i} layers: keypoint_features:{inter_feats[i][:, :, 1:]}; base:{inter_ref_keypoints_unact[i - 1][:, :, 1:]}")

        out_keypoints = paddle.stack(out_keypoints)
        out_logits = paddle.stack(out_logits)

        return (out_keypoints, out_logits, enc_topk_keypoints, enc_topk_logits)
    ...
    def _get_decoder_input(self,
                           ...
                           spatial_shapes,
                           memory_mask=None):
        bs, nq, _ = memory.shape
        # prepare input for decoder
        output_memory, output_anchors = self._get_encoder_output_anchors(
            memory, spatial_shapes, memory_mask)
        enc_outputs_class = self.enc_score_head(output_memory)
        enc_outputs_coord_unact = self.enc_keypoint_head(
            output_memory).reshape([bs, nq, -1, 2]) + output_anchors[..., :2].unsqueeze(-2)
        enc_outputs_center_unact = inverse_sigmoid(paddle.mean(enc_outputs_coord_unact.sigmoid(), axis=2, keepdim=True))
        enc_outputs_coord_unact = paddle.concat([enc_outputs_center_unact, enc_outputs_coord_unact], axis=2).flatten(-2)
        # top-k select index
        ...
        # print(reference_points_unact)
        # exit()
        
        bs, nq, np = reference_points_unact.shape
        reference_points_unact = reference_points_unact.reshape([bs, nq, np // 2, 2])
        enc_topk_keypoints = F.sigmoid(reference_points_unact)[:, :, 1:]
        enc_topk_logits = paddle.gather_nd(enc_outputs_class, topk_ind)
        # extract region features
        tgt = paddle.gather_nd(output_memory, topk_ind).detach().unsqueeze(-2)
        # query construction
        tgt_pose = self.keypoint_embedding.weight[None, None].tile([1, self.num_queries, 1, 1]).expand([bs, -1, -1, -1]) + tgt
        tgt_global = self.instance_embedding.weight[None, None].tile([1, self.num_queries, 1, 1]).expand([bs, -1, -1, -1])
        target = paddle.concat([tgt_global, tgt_pose], axis=2)
        
        return target, reference_points_unact.detach(
            ), enc_topk_keypoints, enc_topk_logits
